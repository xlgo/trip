	...
	<form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<c:url value="${modelPath}list"/>" method="post">
		...
		<div class="searchBar">
			<ul class="searchContent">
				<li style="width: 246px;"><label  style="width: 60px;">国家名称：</label> <input type="text" name="S_areaName" value="${searchParam.S_areaName}" /></li>
			</ul>
			<div class="subBar">
				<ul>
					<li>
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a title="国家维护-添加" class="add" href="<c:url value="${modelPath}add?navTabId=${searchParam.navTabId}"/>" rel="${searchParam.navTabId}_add" target="navTab"><span>添加</span></a></li>
			...
			<li><a title="国家维护-修改" class="edit" href="<c:url value="${modelPath}edit?navTabId=${searchParam.navTabId}&id="/>{modeId}" rel="${searchParam.navTabId}_edit" target="navTab" fresh="false" warn="请选要编辑的行！" ><span>修改</span></a></li>
		</ul>
	</div>
	<table class="table" width="100%" layoutH="138">
		<thead>
			<tr>
				...
				<th orderField="area_name" class="${searchParam.sortField=='area_name'?searchParam.sortOrder:""}">国家名称</th>
			...
			<c:forEach items="${pageList.list}" var="model" varStatus="eachStatus">
				<tr target="modeId" rel="${model.id}"  >
					...
					<td>
						...
						<a title="国家-修改" target="navTab" rel="${searchParam.navTabId}_edit" href="<c:url value="${modelPath}edit?navTabId=${searchParam.navTabId}&id=${model.id}"/>" fresh="false" class="btnEdit">编辑</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
