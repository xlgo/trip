...
<div class="pageContent">
	<form method="post" action="<c:url value="/trip/sysarea/saveJSON"/>" class="pageForm required-validate" onsubmit="return validateCallback(this, navTabAjaxDone);">
		<input type="hidden" name="id" value="${model.id}"/>
		<input type="hidden" name="METHOD" value="${model.METHOD}"/>
		<input type="hidden" name="navTabId" value="${param.navTabId}"/>
		<div  class="pageFormContent" layoutH="56">
			<dl>
				<dt>国家名称：<input name="areaType" type="hidden"  value="4"/></dt>
				<dd>
					<select name="areaPId">
						<c:forEach var="item" items="${sysArea2}">
							<option value="${item.areaId}" ${item.areaId == model.areaPId ?"selected" :""  }>${item.areaName}</option>
						</c:forEach>
					</select>
				</dd>
			</dl>
